  ...
  <div class="appointment-container">
    <el-container>
      ...
      <el-aside width="200px">
        ...
        <el-menu default-active="3" class="el-menu-vertical">
          <el-menu-item index="1" @click="$router.push('/home')">
            <span>首页</span>
          </el-menu-item>
          <el-menu-item index="2" @click="$router.push('/customer')">
            <span>客户管理</span>
          </el-menu-item>
          ...
          <el-card class="search-card">
            <el-form :inline="true" :model="searchForm">
              ...
              <el-form-item label="预约日期">
                <el-date-picker v-model="searchForm.appointmentDate" type="date" placeholder="选择日期" value-format="yyyy-MM-dd" />
              </el-form-item>
            ...
            <el-table :data="tableData" v-loading="loading" stripe>
              ...
              <el-table-column label="操作" fixed="right" width="280">
                <template #default="{ row }">
                  ...
                  <el-button type="text" size="small" @click="handleStartService(row)" v-if="row.status === 2">开始服务</el-button>
                  <el-button type="text" size="small" @click="handleComplete(row)" v-if="row.status === 3">完成</el-button>
                  <el-button type="text" size="small" @click="handleCancel(row)" v-if="row.status !== 4 && row.status !== 5">取消</el-button>
                </template>
              </el-table-column>
            </el-table>
      ...
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px">
        ...
        <el-form-item label="技师" prop="beauticianId">
          <el-select v-model="formData.beauticianId" placeholder="请选择技师">
            <el-option v-for="beautician in beauticianList" :key="beautician.id" :label="beautician.name" :value="beautician.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="预约日期" prop="appointmentDate">
          <el-date-picker v-model="formData.appointmentDate" type="date" placeholder="选择日期" value-format="yyyy-MM-dd" />
        </el-form-item>
        <el-form-item label="预约时间" prop="appointmentTime">
          <el-time-picker v-model="formData.appointmentTime" placeholder="选择时间" format="HH:mm" value-format="HH:mm" />
        </el-form-item>
        <el-form-item label="时长" prop="duration">
          <el-input-number v-model="formData.duration" :min="15" :step="15" /> 分钟
        </el-form-item>
        <el-form-item label="价格" prop="price">
          <el-input-number v-model="formData.price" :min="0" :precision="2" /> 元
        </el-form-item>
    ...
    <!-- 详情弹窗 -->
    <el-dialog title="预约详情" :visible.sync="detailVisible" width="700px">
      ...
      <div style="margin-top: 20px; text-align: center;">
        <el-button type="primary" @click="handleConfirm(detailData)" v-if="detailData && detailData.status === 0">确认预约</el-button>
        <el-button type="success" @click="handleArrive(detailData)" v-if="detailData && detailData.status === 1">客户到店</el-button>
        <el-button type="warning" @click="handleStartService(detailData)" v-if="detailData && detailData.status === 2">开始服务</el-button>
        <el-button type="success" @click="handleComplete(detailData)" v-if="detailData && detailData.status === 3">完成服务</el-button>
        <el-button type="danger" @click="handleCancel(detailData)" v-if="detailData && detailData.status !== 4 && detailData.status !== 5">取消预约</el-button>
      </div>
    </el-dialog>
